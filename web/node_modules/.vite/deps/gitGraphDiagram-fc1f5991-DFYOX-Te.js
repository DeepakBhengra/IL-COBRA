...
import { Hn as require_dist, I as setDiagramTitle, L as setupGraphViewbox, N as setAccDescription, P as setAccTitle, Rn as select_default, T as log$1, Un as require_dayjs_min, _ as getConfig, g as getAccTitle, h as getAccDescription, k as random, o as clear$2, s as common$1, y as getDiagramTitle, z as utils } from "./mermaid-59c9be08-B2V2B2GS.js";
require_dayjs_min();
require_dist();
var parser = function() {
	var o = function(k, v, o2, l) {
		...
		return o2;
	}, $V0 = [1, 3], $V1 = [1, 6], $V2 = [1, 4], $V3 = [1, 5], $V4 = [2, 5], $V5 = [1, 12], $V6 = [
		5,
		7,
		13,
		19,
...
var branch = function(name, order) {
	...
	if (branches[name] === void 0) {
		...
		let error = /* @__PURE__ */ new Error("Trying to create an existing branch. (Help: Either use a new name if you want create a new branch or try using \"checkout " + name + "\")");
		error.hash = {
			text: "branch " + name,
			token: "branch " + name,
			line: "1",
...
var merge = function(otherBranch, custom_id, override_type, custom_tag) {
	...
	if (curBranch === otherBranch) {
		let error = /* @__PURE__ */ new Error("Incorrect usage of \"merge\". Cannot merge a branch to itself");
		error.hash = {
		...
		let error = /* @__PURE__ */ new Error("Incorrect usage of \"merge\". Current branch (" + curBranch + ")has no commits");
		error.hash = {
			text: "merge " + otherBranch,
			token: "merge " + otherBranch,
			line: "1",
		...
		let error = /* @__PURE__ */ new Error("Incorrect usage of \"merge\". Branch to be merged (" + otherBranch + ") does not exist");
		error.hash = {
			text: "merge " + otherBranch,
			token: "merge " + otherBranch,
			line: "1",
		...
		let error = /* @__PURE__ */ new Error("Incorrect usage of \"merge\". Branch to be merged (" + otherBranch + ") has no commits");
		error.hash = {
			text: "merge " + otherBranch,
			token: "merge " + otherBranch,
			line: "1",
		...
		let error = /* @__PURE__ */ new Error("Incorrect usage of \"merge\". Both branches have same head");
		error.hash = {
			text: "merge " + otherBranch,
			token: "merge " + otherBranch,
			line: "1",
		...
		let error = /* @__PURE__ */ new Error("Incorrect usage of \"merge\". Commit with id:" + custom_id + " already exists, use different custom Id");
		error.hash = {
...
var cherryPick = function(sourceId, targetId, tag, parentCommitId) {
	...
	if (!sourceId || commits[sourceId] === void 0) {
		let error = /* @__PURE__ */ new Error("Incorrect usage of \"cherryPick\". Source commit id should exist and provided");
		error.hash = {
	...
	if (parentCommitId && !(Array.isArray(sourceCommit.parents) && sourceCommit.parents.includes(parentCommitId))) throw /* @__PURE__ */ new Error("Invalid operation: The specified parent commit is not an immediate parent of the cherry-picked commit.");
	if (sourceCommit.type === commitType$1.MERGE && !parentCommitId) throw /* @__PURE__ */ new Error("Incorrect usage of cherry-pick: If the source commit is a merge commit, an immediate parent commit must be specified.");
	if (!targetId || commits[targetId] === void 0) {
		if (sourceCommitBranch === curBranch) {
			let error = /* @__PURE__ */ new Error("Incorrect usage of \"cherryPick\". Source commit is already on current branch");
			error.hash = {
				text: "cherryPick " + sourceId + " " + targetId,
				token: "cherryPick " + sourceId + " " + targetId,
				line: "1",
		...
		if (currentCommit === void 0 || !currentCommit) {
			let error = /* @__PURE__ */ new Error("Incorrect usage of \"cherry-pick\". Current branch (" + curBranch + ")has no commits");
			error.hash = {
				text: "cherryPick " + sourceId + " " + targetId,
				token: "cherryPick " + sourceId + " " + targetId,
				line: "1",
...
var checkout = function(branch2) {
	...
	if (branches[branch2] === void 0) {
		let error = /* @__PURE__ */ new Error("Trying to checkout branch which is not yet created. (Help try using \"branch " + branch2 + "\")");
		error.hash = {
...
function prettyPrintCommitHistory(commitArr) {
	...
	for (let branch2 in branches) if (branches[branch2] === commit2.id) label.push(branch2);
...
var getBranchesAsObjArray = function() {
	return Object.values(branchesConfig).map((branchConfig, i) => {
...
var drawText = (txt) => {
	...
	else if (Array.isArray(txt)) rows = txt;
	else rows = [];
...
var drawCommits = (svg, commits2, modifyGraph) => {
	...
	let pos = 0;
	if (dir === "TB") pos = 30;
	const sortedKeys = Object.keys(commits2).sort((a, b) => {
	...
	sortedKeys.forEach((key) => {
		...
		if (isParallelCommits) if (commit2.parents.length) {
		...
		if (modifyGraph) {
			...
			switch (commitSymbolType) {
				...
				case commitType.CHERRY_PICK:
					...
					break;
				default: typeClass = "commit-normal";
			}
			if (commitSymbolType === commitType.HIGHLIGHT) {
				...
				circle.attr("class", `commit ${commit2.id} commit-highlight${branchPos[commit2.branch].index % THEME_COLOR_LIMIT} ${typeClass}-outer`);
				gBullets.append("rect").attr("x", x - 6).attr("y", y - 6).attr("height", 12).attr("width", 12).attr("class", `commit ${commit2.id} commit${branchPos[commit2.branch].index % THEME_COLOR_LIMIT} ${typeClass}-inner`);
				...
				circle.attr("class", `commit ${commit2.id} commit${branchPos[commit2.branch].index % THEME_COLOR_LIMIT}`);
				if (commitSymbolType === commitType.MERGE) {
					const circle2 = gBullets.append("circle");
					circle2.attr("cx", x);
					circle2.attr("cy", y);
					circle2.attr("r", 6);
					circle2.attr("class", `commit ${typeClass} ${commit2.id} commit${branchPos[commit2.branch].index % THEME_COLOR_LIMIT}`);
				}
				if (commitSymbolType === commitType.REVERSE) gBullets.append("path").attr("d", `M ${x - 5},${y - 5}L${x + 5},${y + 5}M${x - 5},${y + 5}L${x + 5},${y - 5}`).attr("class", `commit ${typeClass} ${commit2.id} commit${branchPos[commit2.branch].index % THEME_COLOR_LIMIT}`);
			}
		}
		if (dir === "TB") commitPos[commit2.id] = {
			x,
			y: posWithOffset
		};
		else commitPos[commit2.id] = {
			x: posWithOffset,
			y
		};
		if (modifyGraph) {
			...
			if (commit2.type !== commitType.CHERRY_PICK && (commit2.customId && commit2.type === commitType.MERGE || commit2.type !== commitType.MERGE) && gitGraphConfig.showCommitLabel) {
				...
				if (dir !== "TB") text.attr("x", posWithOffset - bbox.width / 2);
				if (gitGraphConfig.rotateCommitLabel) if (dir === "TB") {
          ...
          ${posWithOffset - tagBbox.width / 2 - px},${ly + h2 + py}`);
            ...
            ${x + layoutOffset + tagBbox.width + px},${pos + h2 + py}
            ${x + layoutOffset},${pos + h2 + py}`).attr("transform", "translate(12,12) rotate(45, " + x + "," + pos + ")");
...
var shouldRerouteArrow = (commitA, commitB, p1, p2, allCommits) => {
	const branchToGetCurve = (dir === "TB" ? p1.x < p2.x : p1.y < p2.y) ? commitB.branch : commitA.branch;
...
var findLane = (y1, y2, depth = 0) => {
	...
	if (lanes.every((lane) => Math.abs(lane - candidate) >= 10)) {
		lanes.push(candidate);
		return candidate;
	}
	return findLane(y1, y2 - Math.abs(y1 - y2) / 5, depth + 1);
};
var drawArrow = (svg, commitA, commitB, allCommits) => {
	...
	if (commitB.type === commitType.MERGE && commitA.id !== commitB.parents[0]) colorClassNum = branchPos[commitA.branch].index;
	let lineDef;
...
var drawBranches = (svg, branches2) => {
	...
	branches2.forEach((branch2, index) => {
		...
		const label = g.insert("g").attr("class", "branchLabel").insert("g").attr("class", "label branch-label" + adjustIndexForTheme);
		...
		if (dir !== "TB") bkg.attr("transform", "translate(-19, " + (pos - bbox.height / 2) + ")");
	});
};
var draw = function(txt, id, ver, diagObj) {
	...
	if (gitGraphConfig.showBranches) drawBranches(diagram2, branches2);
	drawArrows(diagram2, allCommitsDict);
	drawCommits(diagram2, allCommitsDict, true);
	utils.insertTitle(diagram2, "gitTitleText", gitGraphConfig.titleTopMargin, diagObj.db.getDiagramTitle());
	setupGraphViewbox(void 0, diagram2, gitGraphConfig.diagramPadding, gitGraphConfig.useMaxWidth ?? conf.useMaxWidth);
};
var gitGraphRenderer = { draw };
...
//# sourceMappingURL=gitGraphDiagram-fc1f5991-DFYOX-Te.js.map