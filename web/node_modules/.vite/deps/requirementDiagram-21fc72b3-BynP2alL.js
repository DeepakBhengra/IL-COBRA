...
import { Hn as require_dist, N as setAccDescription, P as setAccTitle, Rn as select_default, T as log$1, Un as require_dayjs_min, _ as getConfig, g as getAccTitle, h as getAccDescription, l as configureSvgSize, o as clear$1, s as common$1 } from "./mermaid-59c9be08-B2V2B2GS.js";
...
var parser = function() {
	var o = function(k, v, o2, l) {
		...
		62,
		63
	], $V5 = [1, 18], $V6 = [2, 7], $V7 = [1, 22], $V8 = [1, 23], $V9 = [1, 24], $Va = [1, 25], $Vb = [1, 26], $Vc = [1, 27], $Vd = [1, 20], $Ve = [1, 28], $Vf = [1, 29], $Vg = [62, 63], $Vh = [
		5,
		8,
		9,
		11,
		...
		62,
		63
	], $Vi = [1, 47], $Vj = [1, 48], $Vk = [1, 49], $Vl = [1, 50], $Vm = [1, 51], $Vn = [1, 52], $Vo = [1, 53], $Vp = [53, 54], $Vq = [1, 64], $Vr = [1, 60], $Vs = [1, 61], $Vt = [1, 62], $Vu = [1, 63], $Vv = [1, 65], $Vw = [1, 69], $Vx = [1, 70], $Vy = [1, 67], $Vz = [1, 68], $VA = [
		5,
		8,
		9,
		11,
...
var insertLineEndings = (parentNode, conf2) => {
	...
	parentNode.append("defs").append("marker").attr("id", ReqMarkers.ARROW + "_line_ending").attr("refX", conf2.line_height).attr("refY", .5 * conf2.line_height).attr("markerWidth", conf2.line_height).attr("markerHeight", conf2.line_height).attr("orient", "auto").append("path").attr("d", `M0,0
...
var newTitleNode = (parentNode, id, txts) => {
	...
	txts.forEach((textStr) => {
		...
		else title.append("tspan").attr("text-anchor", "middle").attr("x", conf.rect_min_width / 2).attr("dy", conf.line_height * .75).text(textStr);
		i++;
	});
	let totalY = 1.5 * conf.rect_padding + i * conf.line_height * .75;
...
var addEdgeLabel = (parentNode, svgPath, conf2, txt) => {
	...
	const labelPoint = svgPath.node().getPointAtLength(len * .5);
	const labelId = "rel" + relCnt;
	relCnt++;
	const labelBBox = parentNode.append("text").attr("class", "req relationshipLabel").attr("id", labelId).attr("x", labelPoint.x).attr("y", labelPoint.y).attr("text-anchor", "middle").attr("dominant-baseline", "middle").text(txt).node().getBBox();
...
var drawRelationshipFromLayout = function(svg, rel, g, insert, diagObj) {
	...
	if (rel.type == diagObj.db.Relationships.CONTAINS) svgPath.attr("marker-start", "url(" + common$1.getUrl(conf.arrowMarkerAbsolute) + "#" + rel.type + "_line_ending)");
	else {
		svgPath.attr("stroke-dasharray", "10,7");
		svgPath.attr("marker-end", "url(" + common$1.getUrl(conf.arrowMarkerAbsolute) + "#" + markers.ReqMarkers.ARROW + "_line_ending)");
	}
...
var drawReqs = (reqs, graph, svgNode) => {
	Object.keys(reqs).forEach((reqName) => {
		...
		const rectNode = newRectNode(groupNode, "req-" + reqName);
		let titleNodeInfo = newTitleNode(groupNode, reqName + "_title", [`<<${req.type}>>`, `${req.name}`]);
...
var drawElements = (els, graph, svgNode) => {
	Object.keys(els).forEach((elName) => {
		...
		newBodyNode(groupNode, textId + "_body", [`Type: ${el.type || "Not Specified"}`, `Doc Ref: ${el.docRef || "None"}`], titleNodeInfo.y);
		const rectBBox = rectNode.node().getBBox();
		graph.setNode(id, {
			width: rectBBox.width,
			height: rectBBox.height,
...
var draw = (text, id, _version, diagObj) => {
	...
	const svg = (securityLevel === "sandbox" ? select_default(sandboxElement.nodes()[0].contentDocument.body) : select_default("body")).select(`[id='${id}']`);
...
//# sourceMappingURL=requirementDiagram-21fc72b3-BynP2alL.js.map