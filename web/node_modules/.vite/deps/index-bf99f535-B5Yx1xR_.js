import { Rn as select_default, T as log$1, _ as getConfig, p as evaluate } from "./mermaid-59c9be08-B2V2B2GS.js";
import { D as clone, p as isUndefined, t as Graph, v as map } from "./graphlib-Di4BP18U.js";
import { n as createText } from "./createText-31840e8c-EA-kfJEy.js";
import { c as insertEdgeLabel, d as intersectRect$1, g as updateNodeBounds, h as setNodeElem, i as createLabel$1, l as insertMarkers$1, m as positionNode, n as clear$2, o as getSubGraphTitleMargins, p as positionEdgeLabel, r as clear$1$1, s as insertEdge, u as insertNode } from "./edges-c9cd0ec5-BEHi3uVA.js";
...
//#endregion
//#region node_modules/mermaid/dist/index-bf99f535.js
var clusterDb = {};
var descendants = {};
var parents = {};
var clear$1 = () => {
...
var copy = (clusterId, graph, newGraph, rootId) => {
	log$1.warn("Copying children of ", clusterId, "root", rootId, "data", graph.node(clusterId), rootId);
	...
	nodes.forEach((node) => {
		if (graph.children(node).length > 0) copy(node, graph, newGraph, rootId);
			...
			if (clusterId !== rootId && node !== clusterId) {
				...
				log$1.debug("Not Setting parent for node=", node, "cluster!==rootId", clusterId !== rootId, "node!==clusterId", node !== clusterId);
			}
			...
			edges.forEach((edge) => {
				...
				try {
					...
					} else log$1.info("Skipping copy of edge ", edge.v, "-->", edge.w, " rootId: ", rootId, " clusterId:", clusterId);
				} catch (e) {
					log$1.error(e);
				}
			});
...
var adjustClustersAndEdges = (graph, depth) => {
	if (!graph || depth > 10) {
		...
		return;
	} else log$1.debug("Opting in, graph ");
	graph.nodes().forEach(function(id) {
		if (graph.children(id).length > 0) {
			log$1.warn("Cluster identified", id, " Replacement id in edges: ", findNonClusterChild(id, graph));
	...
	graph.nodes().forEach(function(id) {
		...
		} else log$1.debug("Not a cluster ", id, descendants);
	});
	...
	graph.edges().forEach(function(e) {
		...
		let v = e.v;
		let w = e.w;
		log$1.warn("Fix XXX", clusterDb, "ids:", e.v, e.w, "Translating: ", clusterDb[e.v], " --- ", clusterDb[e.w]);
...
var extractor = (graph, depth) => {
	...
	for (const node of nodes) {
		log$1.debug("Extracting node", node, clusterDb, clusterDb[node] && !clusterDb[node].externalConnections, !graph.parent(node), graph.node(node), graph.children("D"), " Depth ", depth);
		if (!clusterDb[node]) log$1.debug("Not a cluster", node, depth);
		else if (!clusterDb[node].externalConnections && graph.children(node) && graph.children(node).length > 0) {
			log$1.warn("Cluster without external connections, without a parent and with children", node, depth);
			...
			log$1.warn("Cluster ** ", node, " **not meeting the criteria !externalConnections:", !clusterDb[node].externalConnections, " no parent: ", !graph.parent(node), " children ", graph.children(node) && graph.children(node).length > 0, graph.children("D"), depth);
...
var rect = (parent, node) => {
	...
	const text = node.labelType === "markdown" ? createText(label, node.labelText, {
		...
		useHtmlLabels
	}) : label.node().appendChild(createLabel$1(node.labelText, node.labelStyle, void 0, true));
	...
	if (useHtmlLabels) label.attr("transform", `translate(${node.x - bbox.width / 2}, ${node.y - node.height / 2 + subGraphTitleTopMargin})`);
	else label.attr("transform", `translate(${node.x}, ${node.y - node.height / 2 + subGraphTitleTopMargin})`);
...
var roundedWithTitle = (parent, node) => {
	...
	node.height = rect2.node().getBBox().height;
	node.intersect = function(point) {
		return intersectRect$1(node, point);
	};
	return shapeSvg;
...
var recursiveRender = async (_elem, graph, diagramType, id, parentCluster, siteConfig) => {
	...
	if (!graph.nodes()) log$1.info("No nodes found for", graph);
	else log$1.info("Recursive render XXX", graph.nodes());
	if (graph.edges().length > 0) log$1.trace("Recursive edges", graph.edge(graph.edges()[0]));
	...
	sortNodesByHierarchy(graph).forEach(function(v) {
		...
		log$1.info("Position " + v + ": (" + node.x, "," + node.y, ") width: ", node.width, " height: ", node.height);
		if (node && node.clusterNode) {
	...
	graph.edges().forEach(function(e) {
		...
		positionEdgeLabel(edge, insertEdge(edgePaths, e, edge, clusterDb, diagramType, graph, id));
	});
...
var render = async (elem, graph, markers, diagramType, id) => {
	...
	await recursiveRender(elem, graph, diagramType, id, void 0, getConfig());
};
//#endregion
export { render as t };

//# sourceMappingURL=index-bf99f535-B5Yx1xR_.js.map