import "./chunk-B4lejLz5.js";
import { A as renderKatexSanitized, D as parseFontSize, Hn as require_dist, I as setDiagramTitle, N as setAccDescription, P as setAccTitle, Rn as select_default, S as hasKatex, T as log$1, Un as require_dayjs_min, _ as getConfig, g as getAccTitle, h as getAccDescription, j as sanitizeText$2, l as configureSvgSize, n as calculateMathMLDimensions, o as clear$1, s as common$1, t as assignWithDepth$1, v as getConfig$1, y as getDiagramTitle, z as utils } from "./mermaid-59c9be08-B2V2B2GS.js";
import { i as drawRect$1, n as drawEmbeddedImage, o as getNoteRect$1, r as drawImage, s as getTextObj$1, t as drawBackgroundRect$1 } from "./svgDrawCommon-70a69047-BCzqiMOK.js";
//#region node_modules/mermaid/dist/sequenceDiagram-40e752f5.js
...
var parser = function() {
	var o = function(k, v, o2, l) {
		...
		return o2;
	}, $V0 = [1, 2], $V1 = [1, 3], $V2 = [1, 4], $V3 = [2, 4], $V4 = [1, 9], $V5 = [1, 11], $V6 = [1, 13], $V7 = [1, 14], $V8 = [1, 16], $V9 = [1, 17], $Va = [1, 18], $Vb = [1, 24], $Vc = [1, 25], $Vd = [1, 26], $Ve = [1, 27], $Vf = [1, 28], $Vg = [1, 29], $Vh = [1, 30], $Vi = [1, 31], $Vj = [1, 32], $Vk = [1, 33], $Vl = [1, 34], $Vm = [1, 35], $Vn = [1, 36], $Vo = [1, 37], $Vp = [1, 38], $Vq = [1, 39], $Vr = [1, 41], $Vs = [1, 42], $Vt = [1, 43], $Vu = [1, 44], $Vv = [1, 45], $Vw = [1, 46], $Vx = [
		1,
		4,
		5,
		13,
	...
	var parser2 = {
		...
		parse: function parse(input) {
			...
			while (true) {
				...
				if (this.defaultActions[state2]) action = this.defaultActions[state2];
				else {
				...
				if (typeof action === "undefined" || !action.length || !action[0]) {
					var errStr = "";
					expected = [];
					for (p in table[state2]) if (this.terminals_[p] && p > TERROR) expected.push("'" + this.terminals_[p] + "'");
				...
				if (action[0] instanceof Array && action.length > 1) throw new Error("Parse Error: multiple actions possible at state: " + state2 + ", token: " + symbol);
				switch (action[0]) {
					case 1:
						stack.push(symbol);
...
var addActor = function(id, name, description, type) {
	...
	if (old) {
		if (state.records.currentBox && old.box && state.records.currentBox !== old.box) throw new Error("A same participant should only be defined in one Box: " + old.name + " can't be in '" + old.box.name + "' and in '" + state.records.currentBox.name + "' at the same time.");
	...
	if (state.records.prevActor && state.records.actors[state.records.prevActor]) state.records.actors[state.records.prevActor].nextActor = id;
	if (state.records.currentBox) state.records.currentBox.actorKeys.push(id);
...
var activationCount = (part) => {
	...
	for (i = 0; i < state.records.messages.length; i++) {
		if (state.records.messages[i].type === LINETYPE.ACTIVE_START && state.records.messages[i].from.actor === part) count++;
		if (state.records.messages[i].type === LINETYPE.ACTIVE_END && state.records.messages[i].from.actor === part) count--;
	}
	return count;
};
...
var addSignal = function(idFrom, idTo, message = {
	text: void 0,
	wrap: void 0
}, messageType, activate = false) {
	if (messageType === LINETYPE.ACTIVE_END) {
		if (activationCount(idFrom.actor) < 1) {
			let error = /* @__PURE__ */ new Error("Trying to inactivate an inactive participant (" + idFrom.actor + ")");
			error.hash = {
				text: "->>-",
				token: "->>-",
				line: "1",
...
var autoWrap = () => {
	if (state.records.wrapEnabled !== void 0) return state.records.wrapEnabled;
...
var addLinks = function(actorId, text) {
	...
	try {
		...
		insertLinks(actor, JSON.parse(sanitizedText));
	} catch (e) {
...
var addProperties = function(actorId, text) {
	...
	try {
		let sanitizedText = sanitizeText$2(text.text, getConfig());
		insertProperties(actor, JSON.parse(sanitizedText));
	} catch (e) {
...
var apply = function(param) {
	if (Array.isArray(param)) param.forEach(function(item) {
		...
		case "createParticipant":
			if (state.records.actors[param.actor]) throw new Error("It is not possible to have actors with the same id, even if one is destroyed before the next is created. Use 'AS' aliases to simulate the behavior");
		...
		case "addDetails":
			...
			break;
		case "addMessage":
			if (state.records.lastCreated) if (param.to !== state.records.lastCreated) throw new Error("The created participant " + state.records.lastCreated + " does not have an associated creating message after its declaration. Please check the sequence diagram.");
			else state.records.lastCreated = void 0;
			else if (state.records.lastDestroyed) if (param.to !== state.records.lastDestroyed && param.from !== state.records.lastDestroyed) throw new Error("The destroyed participant " + state.records.lastDestroyed + " does not have an associated destroying message after its declaration. Please check the sequence diagram.");
			else state.records.lastDestroyed = void 0;
...
var drawPopup = function(elem, actor, minMenuWidth, textAttrs, forceMenus) {
	...
	if (links != null) {
		...
		for (let key in links) {
			...
			var sanitizedLink = (0, import_dist.sanitizeUrl)(links[key]);
...
var drawKatex = async function(elem, textData, msgModel = null) {
	...
	const dim = textElem.append("xhtml:div").attr("style", "width: fit-content;").attr("xmlns", "http://www.w3.org/1999/xhtml").html(linesSanitized).node().getBoundingClientRect();
	...
	if (textData.class === "noteText") {
		...
		if (textData.class === "loopText") textElem.attr("y", Math.round(starty));
		else textElem.attr("y", Math.round(starty - dim.height));
	}
	return [textElem];
};
...
var fixLifeLineHeights = (diagram2, actors, actorKeys, conf2) => {
	if (!diagram2.select) return;
	actorKeys.forEach((actorKey) => {
		const actor = actors[actorKey];
		const actorDOM = diagram2.select("#actor" + actor.actorCnt);
		if (!conf2.mirrorActors && actor.stopy) actorDOM.attr("y2", actor.stopy + actor.height / 2);
		else if (conf2.mirrorActors) actorDOM.attr("y2", actor.stopy);
	});
};
var drawActorTypeParticipant = async function(elem, actor, conf2, isFooter) {
	...
	if (!isFooter) {
		actorCnt++;
		if (Object.keys(actor.links || {}).length && !conf2.forceMenus) g.attr("onclick", popupMenuToggle(`actor${actorCnt}_popup`)).attr("cursor", "pointer");
		...
		if (actor.links != null) g.attr("id", "root-" + actorCnt);
	}
	...
	if (isFooter) cssclass += ` ${BOTTOM_ACTOR_CLASS}`;
	else cssclass += ` ${TOP_ACTOR_CLASS}`;
	rect.x = actor.x;
	rect.y = actorY;
	...
	if (actor.properties != null && actor.properties["icon"]) {
		...
		if (iconSrc.charAt(0) === "@") drawEmbeddedImage(g, rect.x + rect.width - 20, rect.y + 10, iconSrc.substr(1));
		else drawImage(g, rect.x + rect.width - 20, rect.y + 10, iconSrc);
	}
	await _drawTextCandidateFunc(conf2, hasKatex(actor.description))(actor.description, g, rect.x, rect.y, rect.width, rect.height, { class: "actor" }, conf2);
...
var drawActorTypeActor = async function(elem, actor, conf2, isFooter) {
	...
	actor.height = actElem.node().getBBox().height;
	await _drawTextCandidateFunc(conf2, hasKatex(actor.description))(actor.description, actElem, rect.x, rect.y + 35, rect.width, rect.height, { class: "actor" }, conf2);
	return actor.height;
};
var drawActor = async function(elem, actor, conf2, isFooter) {
	switch (actor.type) {
		case "actor": return await drawActorTypeActor(elem, actor, conf2, isFooter);
		case "participant": return await drawActorTypeParticipant(elem, actor, conf2, isFooter);
	}
};
var drawBox = async function(elem, box, conf2) {
	const g = elem.append("g");
	drawBackgroundRect(g, box);
	if (box.name) await _drawTextCandidateFunc(conf2)(box.name, g, box.x, box.y + (box.textMaxHeight || 0) / 2, box.width, 0, { class: "text" }, conf2);
	g.lower();
};
...
var drawLoop = async function(elem, loopModel, labelText, conf2) {
	...
	if (loopModel.sectionTitles !== void 0) {
		for (const [idx, item] of Object.entries(loopModel.sectionTitles)) if (item.message) {
...
var insertArrowCrossHead = function(elem) {
	elem.append("defs").append("marker").attr("id", "crosshead").attr("markerWidth", 15).attr("markerHeight", 8).attr("orient", "auto").attr("refX", 4).attr("refY", 4.5).append("path").attr("fill", "none").attr("stroke", "#000000").style("stroke-dasharray", "0, 0").attr("stroke-width", "1pt").attr("d", "M 1,2 L 6,7 M 6,2 L 1,7");
};
...
var _drawTextCandidateFunc = function() {
	function byText(content, g, x, y, width, height, textAttrs) {
		_setTextAttrs(g.append("text").attr("x", x + width / 2).attr("y", y + height / 2 + 5).style("text-anchor", "middle").text(content), textAttrs);
	}
	...
	function byFo(content, g, x, y, width, height, textAttrs, conf2) {
		...
		const text = s.append("foreignObject").attr("x", x).attr("y", y).attr("width", width).attr("height", height).append("xhtml:div").style("display", "table").style("height", "100%").style("width", "100%");
	...
	async function byKatex(content, g, x, y, width, height, textAttrs, conf2) {
		...
		const s = g.append("switch");
		const text = s.append("foreignObject").attr("x", x + width / 2 - dim.width / 2).attr("y", y + height / 2 - dim.height / 2).attr("width", dim.width).attr("height", dim.height).append("xhtml:div").style("height", "100%").style("width", "100%");
	...
	return function(conf2, hasKatex2 = false) {
		if (hasKatex2) return byKatex;
		return conf2.textPlacement === "fo" ? byFo : conf2.textPlacement === "old" ? byText : byTspan;
	};
}();
var _drawMenuItemTextCandidateFunc = function() {
	function byText(content, g, x, y, width, height, textAttrs) {
		_setTextAttrs(g.append("text").attr("x", x).attr("y", y).style("text-anchor", "start").text(content), textAttrs);
	}
	function byTspan(content, g, x, y, width, height, textAttrs, conf2) {
		const { actorFontSize, actorFontFamily, actorFontWeight } = conf2;
		const lines = content.split(common$1.lineBreakRegex);
...
var drawMessage = async function(diagram2, msgModel, lineStartY, diagObj) {
	...
	textObj.tspan = false;
	hasKatex(textObj.text) ? await drawKatex(diagram2, textObj, {
		startx,
		stopx,
		starty: lineStartY
	...
	const textWidth = textDims.width;
	let line;
	if (startx === stopx) if (conf.rightAngles) line = diagram2.append("path").attr("d", `M  ${startx},${lineStartY} H ${startx + common$1.getMax(conf.width / 2, textWidth / 2)} V ${lineStartY + 25} H ${startx}`);
...
var addActorRenderingData = async function(diagram2, actors, createdActors, actorKeys, verticalPos, messages, isFooter) {
	...
	for (const actorKey of actorKeys) {
		...
		if (createdActors[actor.name]) prevMargin += actor.width / 2;
...
var drawActors = async function(diagram2, actors, actorKeys, isFooter) {
	if (!isFooter) for (const actorKey of actorKeys) {
		const actor = actors[actorKey];
		...
		for (const actorKey of actorKeys) {
			const actor = actors[actorKey];
			if (!actor.stopy) actor.stopy = bounds.getVerticalPos();
...
var activationBounds = function(actor, actors) {
	...
	return [activations.reduce(function(acc, activation) {
		return common$1.getMin(acc, activation.startx);
	}, actorObj.x + actorObj.width / 2 - 1), activations.reduce(function(acc, activation) {
...
function adjustCreatedDestroyedData(msg, msgModel, lineStartY, index, actors, createdActors, destroyedActors) {
	...
	if (createdActors[msg.to] == index) {
		...
		receiverAdjustment(actor, actor.type == "actor" ? 21 : actor.width / 2 + 3);
		actor.starty = lineStartY - actor.height / 2;
		bounds.bumpVerticalPos(actor.height / 2);
	} else if (destroyedActors[msg.from] == index) {
		const actor = actors[msg.from];
		if (conf.mirrorActors) senderAdjustment(actor, actor.type == "actor" ? ACTOR_TYPE_WIDTH / 2 : actor.width / 2);
		actor.stopy = lineStartY - actor.height / 2;
		bounds.bumpVerticalPos(actor.height / 2);
	} else if (destroyedActors[msg.to] == index) {
		const actor = actors[msg.to];
		if (conf.mirrorActors) receiverAdjustment(actor, actor.type == "actor" ? 21 : actor.width / 2 + 3);
		actor.stopy = lineStartY - actor.height / 2;
		bounds.bumpVerticalPos(actor.height / 2);
	}
}
var draw = async function(_text, id, _version, diagObj) {
	...
	if (hasBoxes) {
		bounds.bumpVerticalPos(conf.boxMargin);
		if (hasBoxTitles) bounds.bumpVerticalPos(boxes[0].textMaxHeight);
	}
	...
	for (const e of messagesToDraw) await drawMessage(diagram2, e.messageModel, e.lineStartY, diagObj);
	if (conf.mirrorActors) await drawActors(diagram2, actors, actorKeys, true);
...
async function getMaxMessageWidthPerActor(actors, messages, diagObj) {
	...
	for (const msg of messages) if (actors[msg.to] && actors[msg.from]) {
		const actor = actors[msg.to];
		if (msg.placement === diagObj.db.PLACEMENT.LEFTOF && !actor.prevActor) continue;
		if (msg.placement === diagObj.db.PLACEMENT.RIGHTOF && !actor.nextActor) continue;
		...
		const messageWidth = (hasKatex(wrappedMessage) ? await calculateMathMLDimensions(msg.message, getConfig()) : utils.calculateTextDimensions(wrappedMessage, textFont)).width + 2 * conf.wrapPadding;
		if (isMessage && msg.from === actor.nextActor) maxMessageWidthPerActor[msg.to] = common$1.getMax(maxMessageWidthPerActor[msg.to] || 0, messageWidth);
		else if (isMessage && msg.from === actor.prevActor) maxMessageWidthPerActor[msg.from] = common$1.getMax(maxMessageWidthPerActor[msg.from] || 0, messageWidth);
		else if (isMessage && msg.from === msg.to) {
			maxMessageWidthPerActor[msg.from] = common$1.getMax(maxMessageWidthPerActor[msg.from] || 0, messageWidth / 2);
			maxMessageWidthPerActor[msg.to] = common$1.getMax(maxMessageWidthPerActor[msg.to] || 0, messageWidth / 2);
		} else if (msg.placement === diagObj.db.PLACEMENT.RIGHTOF) maxMessageWidthPerActor[msg.from] = common$1.getMax(maxMessageWidthPerActor[msg.from] || 0, messageWidth);
		else if (msg.placement === diagObj.db.PLACEMENT.LEFTOF) maxMessageWidthPerActor[actor.prevActor] = common$1.getMax(maxMessageWidthPerActor[actor.prevActor] || 0, messageWidth);
		else if (msg.placement === diagObj.db.PLACEMENT.OVER) {
			if (actor.prevActor) maxMessageWidthPerActor[actor.prevActor] = common$1.getMax(maxMessageWidthPerActor[actor.prevActor] || 0, messageWidth / 2);
			if (actor.nextActor) maxMessageWidthPerActor[msg.from] = common$1.getMax(maxMessageWidthPerActor[msg.from] || 0, messageWidth / 2);
		}
	}
...
var getRequiredPopupWidth = function(actor) {
	...
	for (const key in actor.links) {
		const labelWidth = utils.calculateTextDimensions(key, textFont).width + 2 * conf.wrapPadding + 2 * conf.boxMargin;
...
async function calculateActorMargins(actors, actorToMessageWidth, boxes) {
	...
	for (const prop of Object.keys(actors)) {
		...
		if (actor.wrap) actor.description = utils.wrapLabel(actor.description, conf.width - 2 * conf.wrapPadding, actorFont(conf));
	...
	for (const actorKey in actorToMessageWidth) {
		...
		if (!nextActor) {
			const actorWidth2 = actorToMessageWidth[actorKey] + conf.actorMargin - actor.width / 2;
			actor.margin = common$1.getMax(actorWidth2, conf.actorMargin);
			continue;
		}
		const actorWidth = actorToMessageWidth[actorKey] + conf.actorMargin - actor.width / 2 - nextActor.width / 2;
	...
	boxes.forEach((box) => {
		...
		if (box.wrap) box.name = utils.wrapLabel(box.name, totalWidth - 2 * conf.wrapPadding, textFont);
...
var buildNoteModel = async function(msg, actors, diagObj) {
	...
	let textDimensions = hasKatex(msg.message) ? await calculateMathMLDimensions(msg.message, getConfig()) : utils.calculateTextDimensions(shouldWrap ? utils.wrapLabel(msg.message, conf.width, noteFont(conf)) : msg.message, noteFont(conf));
	const noteModel = {
	...
	if (msg.placement === diagObj.db.PLACEMENT.RIGHTOF) {
		noteModel.width = shouldWrap ? common$1.getMax(conf.width, textDimensions.width) : common$1.getMax(actors[msg.from].width / 2 + actors[msg.to].width / 2, textDimensions.width + 2 * conf.noteMargin);
		noteModel.startx = startx + (actors[msg.from].width + conf.actorMargin) / 2;
	} else if (msg.placement === diagObj.db.PLACEMENT.LEFTOF) {
		noteModel.width = shouldWrap ? common$1.getMax(conf.width, textDimensions.width + 2 * conf.noteMargin) : common$1.getMax(actors[msg.from].width / 2 + actors[msg.to].width / 2, textDimensions.width + 2 * conf.noteMargin);
		noteModel.startx = startx - noteModel.width + (actors[msg.from].width - conf.actorMargin) / 2;
	} else if (msg.to === msg.from) {
		textDimensions = utils.calculateTextDimensions(shouldWrap ? utils.wrapLabel(msg.message, common$1.getMax(conf.width, actors[msg.from].width), noteFont(conf)) : msg.message, noteFont(conf));
		noteModel.width = shouldWrap ? common$1.getMax(conf.width, actors[msg.from].width) : common$1.getMax(actors[msg.from].width, conf.width, textDimensions.width + 2 * conf.noteMargin);
	...
	if (shouldWrap) noteModel.message = utils.wrapLabel(msg.message, noteModel.width - 2 * conf.wrapPadding, noteFont(conf));
	log$1.debug(`NM:[${noteModel.startx},${noteModel.stopx},${noteModel.starty},${noteModel.stopy}:${noteModel.width},${noteModel.height}=${msg.message}]`);
	return noteModel;
};
var buildMessageModel = function(msg, actors, diagObj) {
	...
	if (msg.from === msg.to) stopx = startx;
	else {
		if (msg.activate && !isArrowToActivation) stopx += adjustValue(conf.activationWidth / 2 - 1);
		if (![diagObj.db.LINETYPE.SOLID_OPEN, diagObj.db.LINETYPE.DOTTED_OPEN].includes(msg.type)) stopx += adjustValue(3);
	}
	const allBounds = [
		fromLeft,
		fromRight,
	...
	if (msg.wrap && msg.message) msg.message = utils.wrapLabel(msg.message, common$1.getMax(boundedWidth + 2 * conf.wrapPadding, conf.width), messageFont(conf));
	const msgDims = utils.calculateTextDimensions(msg.message, messageFont(conf));
	return {
		width: common$1.getMax(msg.wrap ? 0 : msgDims.width + 2 * conf.wrapPadding, boundedWidth + 2 * conf.wrapPadding, conf.width),
		height: 0,
		startx,
		stopx,
		starty: 0,
		...
		if (msg.placement !== void 0) {
			...
			if (msgModel.startx && msgModel.stopx && stack.length > 0) stack.forEach((stk) => {
				...
				if (msgModel.startx === msgModel.stopx) {
					...
					current.from = common$1.getMin(from.x - msgModel.width / 2, from.x - from.width / 2, current.from);
					current.to = common$1.getMax(to.x + msgModel.width / 2, to.x + from.width / 2, current.to);
...
//# sourceMappingURL=sequenceDiagram-40e752f5-Blem9CXj.js.map