...
import { Hn as require_dist, O as parseGenericTypes, Rn as select_default, Rt as basis_default, T as log$1, Un as require_dayjs_min, _ as getConfig, l as configureSvgSize, z as utils } from "./mermaid-59c9be08-B2V2B2GS.js";
import { t as line_default } from "./line-C-P71smO.js";
import { t as Graph } from "./graphlib-Di4BP18U.js";
import { n as parser$1, r as styles, t as db } from "./styles-690bd5e1-CGpHdKSx.js";
...
var drawEdge = function(elem, path, relation, conf, diagObj) {
	const getRelationType = function(type) {
		switch (type) {
			case diagObj.db.relationType.AGGREGATION: return "aggregation";
			case diagObj.db.relationType.EXTENSION: return "extension";
			case diagObj.db.relationType.COMPOSITION: return "composition";
			case diagObj.db.relationType.DEPENDENCY: return "dependency";
			case diagObj.db.relationType.LOLLIPOP: return "lollipop";
		}
	};
	...
	if (relation.relationTitle1 !== void 0 && relation.relationTitle1 !== "none") elem.append("g").attr("class", "cardinality").append("text").attr("class", "type1").attr("x", p1_card_x).attr("y", p1_card_y).attr("fill", "black").attr("font-size", "6").text(relation.relationTitle1);
	if (relation.relationTitle2 !== void 0 && relation.relationTitle2 !== "none") elem.append("g").attr("class", "cardinality").append("text").attr("class", "type2").attr("x", p2_card_x).attr("y", p2_card_y).attr("fill", "black").attr("font-size", "6").text(relation.relationTitle2);
	edgeCount++;
};
var drawClass = function(elem, classDef, conf, diagObj) {
	...
	const rectWidth = g.insert("rect", ":first-child").attr("x", 0).attr("y", 0).attr("width", classBox.width + 2 * conf.padding).attr("height", classBox.height + conf.padding + .5 * conf.dividerMargin).attr("class", cssClassStr).node().getBBox().width;
	...
	classInfo.height = classBox.height + conf.padding + .5 * conf.dividerMargin;
	return classInfo;
};
var getClassTitleString = function(classDef) {
	let classTitleString = classDef.id;
	if (classDef.type) classTitleString += "<" + parseGenericTypes(classDef.type) + ">";
...
var drawNote = function(elem, note, conf, diagObj) {
	...
	const rectWidth = g.insert("rect", ":first-child").attr("x", 0).attr("y", 0).attr("width", noteBox.width + 2 * conf.padding).attr("height", noteBox.height + lines.length * conf.textHeight + conf.padding + .5 * conf.dividerMargin).node().getBBox().width;
	text.node().childNodes.forEach(function(x) {
		x.setAttribute("x", (rectWidth - x.getBBox().width) / 2);
	});
	noteInfo.width = rectWidth;
	noteInfo.height = noteBox.height + lines.length * conf.textHeight + conf.padding + .5 * conf.dividerMargin;
	return noteInfo;
};
...
var draw = function(text, id, _version, diagObj) {
	...
	diagObj.db.getRelations().forEach(function(relation) {
		log$1.info("tjoho" + getGraphId(relation.id1) + getGraphId(relation.id2) + JSON.stringify(relation));
		g.setEdge(getGraphId(relation.id1), getGraphId(relation.id2), { relation }, relation.title || "DEFAULT");
	});
	diagObj.db.getNotes().forEach(function(note) {
		log$1.debug(`Adding note: ${JSON.stringify(note)}`);
		const node = svgDraw.drawNote(diagram2, note, conf, diagObj);
		idCache[node.id] = node;
		g.setNode(node.id, node);
		if (note.class && note.class in classes) g.setEdge(note.id, getGraphId(note.class), { relation: {
			id1: note.id,
			id2: note.class,
			relation: {
				type1: "none",
...
//# sourceMappingURL=classDiagram-e8fde69c-yuJTJlHJ.js.map