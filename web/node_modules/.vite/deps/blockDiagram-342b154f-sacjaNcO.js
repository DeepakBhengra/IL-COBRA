...
import { Ft as rgba, Hn as require_dist, Rn as select_default, T as log$1, Un as require_dayjs_min, _ as getConfig, b as getStylesFromArray, l as configureSvgSize, o as clear$1, s as common$1, v as getConfig$1 } from "./mermaid-59c9be08-B2V2B2GS.js";
import { t as ordinal } from "./ordinal-CwuWvYe0.js";
import { t as Tableau10_default } from "./Tableau10-CecTvLwU.js";
import { t as channel } from "./channel-BT2qUpdH.js";
import { D as clone, t as Graph } from "./graphlib-Di4BP18U.js";
import { c as insertEdgeLabel, l as insertMarkers$1, m as positionNode, p as positionEdgeLabel, s as insertEdge, u as insertNode } from "./edges-c9cd0ec5-BEHi3uVA.js";
require_dayjs_min();
require_dist();
var _a, _b;
var parser = function() {
	var o = function(k, v, o2, l) {
		...
		return o2;
	}, $V0 = [1, 7], $V1 = [1, 13], $V2 = [1, 14], $V3 = [1, 15], $V4 = [1, 19], $V5 = [1, 16], $V6 = [1, 17], $V7 = [1, 18], $V8 = [8, 30], $V9 = [
		8,
		21,
		28,
		29,
...
var addStyleClass = function(id, styleAttributes = "") {
	...
	if (styleAttributes !== void 0 && styleAttributes !== null) styleAttributes.split(STYLECLASS_SEP).forEach((attrib) => {
		...
		if (attrib.match(COLOR_KEYWORD)) {
			const newStyle2 = fixedAttrib.replace(FILL_KEYWORD, BG_FILL).replace(COLOR_KEYWORD, FILL_KEYWORD);
...
var populateBlockDatabase = (_blockList, parent) => {
	...
	for (const block of blockList) {
		...
		if (block.type === "applyStyles") {
			if (block == null ? void 0 : block.stylesStr) addStyle2Node(block.id, block == null ? void 0 : block.stylesStr);
			continue;
		}
		if (block.type === "column-setting") parent.columns = block.columns || -1;
		else if (block.type === "edge") {
			if (edgeCount[block.id]) edgeCount[block.id]++;
			else edgeCount[block.id] = 1;
			...
			if (newBlock) blockDatabase[block.id] = block;
			else {
				if (block.type !== "na") blockDatabase[block.id].type = block.type;
				if (block.label !== block.id) blockDatabase[block.id].label = block.label;
			}
...
var fade = (color, opacity) => {
	...
	return rgba(channel$1(color, "r"), channel$1(color, "g"), channel$1(color, "b"), opacity);
};
var getStyles = (options) => `.label {
  ...
  .labelBkg {
    background-color: ${fade(options.edgeLabelBackground, .5)};
    // background-color:
  }

  .node .cluster {
    // fill: ${fade(options.mainBkg, .5)};
    fill: ${fade(options.clusterBkg, .5)};
    stroke: ${fade(options.clusterBorder, .2)};
...
async function insertBlockPositioned(elem, block, db2) {
	...
	if (db2.getBlock(node.id).type !== "space") {
...
function calculateBlockPosition(columns, position) {
	...
	return {
		px: position % columns,
		py: Math.floor(position / columns)
	};
}
var getMaxChildSize = (block) => {
	...
	for (const child of block.children) {
		...
		log$1.debug("getMaxChildSize abc95 child:", child.id, "width:", width, "height:", height, "x:", x, "y:", y, child.type);
...
function setBlockSizes(block, db2, siblingWidth = 0, siblingHeight = 0) {
	...
	log$1.debug("setBlockSizes abc95 (start)", block.id, (_a2 = block == null ? void 0 : block.size) == null ? void 0 : _a2.x, "block width =", block == null ? void 0 : block.size, "sieblingWidth", siblingWidth);
	...
	if (((_c = block.children) == null ? void 0 : _c.length) > 0) {
		...
		for (const child of block.children) if (child.size) {
			log$1.debug(`abc95 Setting size of children of ${block.id} id=${child.id} ${maxWidth} ${maxHeight} ${child.size}`);
			child.size.width = maxWidth * (child.widthInColumns || 1) + padding * ((child.widthInColumns || 1) - 1);
			child.size.height = maxHeight;
			child.size.x = 0;
			child.size.y = 0;
			log$1.debug(`abc95 updating size of ${block.id} children child:${child.id} maxWidth:${maxWidth} maxHeight:${maxHeight}`);
		}
		...
		if (width < siblingWidth) {
			log$1.debug(`Detected to small siebling: abc95 ${block.id} sieblingWidth ${siblingWidth} sieblingHeight ${siblingHeight} width ${width}`);
			width = siblingWidth;
			height = siblingHeight;
		...
		log$1.debug(`abc95 (finale calc) ${block.id} xSize ${xSize} ySize ${ySize} columns ${columns}${block.children.length} width=${Math.max(width, ((_d = block.size) == null ? void 0 : _d.width) || 0)}`);
	...
	log$1.debug("setBlockSizes abc94 (done)", block.id, (_h = block == null ? void 0 : block.size) == null ? void 0 : _h.x, (_i = block == null ? void 0 : block.size) == null ? void 0 : _i.width, (_j = block == null ? void 0 : block.size) == null ? void 0 : _j.y, (_k = block == null ? void 0 : block.size) == null ? void 0 : _k.height);
}
function layoutBlocks(block, db2) {
	var _a2, _b2, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m, _n, _o, _p, _q;
	log$1.debug(`abc85 layout blocks (=>layoutBlocks) ${block.id} x: ${(_a2 = block == null ? void 0 : block.size) == null ? void 0 : _a2.x} y: ${(_b2 = block == null ? void 0 : block.size) == null ? void 0 : _b2.y} width: ${(_c = block == null ? void 0 : block.size) == null ? void 0 : _c.width}`);
	const columns = block.columns || -1;
	...
	if (block.children && block.children.length > 0) {
		...
		for (const child of block.children) {
			...
			log$1.debug(`abc89 layout blocks (child) id: ${child.id} Pos: ${columnPos} (px, py) ${px},${py} (${(_m = parent == null ? void 0 : parent.size) == null ? void 0 : _m.x},${(_n = parent == null ? void 0 : parent.size) == null ? void 0 : _n.y}) parent: ${parent.id} width: ${width2}${padding}`);
			if (parent.size) {
				const halfWidth = width2 / 2;
				child.size.x = startingPosX + padding + halfWidth;
				log$1.debug(`abc91 layout blocks (calc) px, pyid:${child.id} startingPos=X${startingPosX} new startingPosX${child.size.x} ${halfWidth} padding=${padding} width=${width2} halfWidth=${halfWidth} => x:${child.size.x} y:${child.size.y} ${child.widthInColumns} (width * (child?.w || 1)) / 2 ${width2 * ((child == null ? void 0 : child.widthInColumns) || 1) / 2}`);
				startingPosX = child.size.x + halfWidth;
				child.size.y = parent.size.y - parent.size.height / 2 + py * (height + padding) + height / 2 + padding;
				log$1.debug(`abc88 layout blocks (calc) px, pyid:${child.id}startingPosX${startingPosX}${padding}${halfWidth}=>x:${child.size.x}y:${child.size.y}${child.widthInColumns}(width * (child?.w || 1)) / 2${width2 * ((child == null ? void 0 : child.widthInColumns) || 1) / 2}`);
			}
			if (child.children) layoutBlocks(child);
			columnPos += (child == null ? void 0 : child.widthInColumns) || 1;
			log$1.debug("abc88 columnsPos", child, columnPos);
		}
	}
	log$1.debug(`layout blocks (<==layoutBlocks) ${block.id} x: ${(_o = block == null ? void 0 : block.size) == null ? void 0 : _o.x} y: ${(_p = block == null ? void 0 : block.size) == null ? void 0 : _p.y} width: ${(_q = block == null ? void 0 : block.size) == null ? void 0 : _q.width}`);
}
...
var draw = async function(text, id, _version, diagObj) {
	...
	if (bounds) {
		...
		const magicFactor = Math.max(1, Math.round(.125 * (bounds2.width / bounds2.height)));
...
//# sourceMappingURL=blockDiagram-342b154f-sacjaNcO.js.map