...
import { Hn as require_dist, Mt as darken, Nt as lighten, Pt as isDark, R as setupGraphViewbox$1, Rn as select_default, T as log$1, Un as require_dayjs_min, _ as getConfig, c as commonDb, o as clear$1 } from "./mermaid-59c9be08-B2V2B2GS.js";
import { t as arc_default } from "./arc-zOgpRmFV.js";
require_dayjs_min();
require_dist();
var parser = function() {
	var o = function(k, v, o2, l) {
		...
		20,
		21
	], $V1 = [1, 9], $V2 = [1, 10], $V3 = [1, 11], $V4 = [1, 12], $V5 = [1, 13], $V6 = [1, 16], $V7 = [1, 17];
	var parser2 = {
...
var addEvent = function(event) {
	rawTasks.find((task) => task.id === currentTaskId - 1).events.push(event);
};
...
var drawLabel = function(elem, txtObject) {
	...
	txtObject.x = txtObject.x + .5 * txtObject.labelMargin;
...
var drawTask = function(elem, task, conf) {
	...
	const g = elem.append("g");
	taskCount++;
	g.append("line").attr("id", "task" + taskCount).attr("x1", center).attr("y1", task.y).attr("x2", center).attr("y2", 450).attr("class", "task-line").attr("stroke-width", "1px").attr("stroke-dasharray", "4 2").attr("stroke", "#666");
	drawFace(g, {
		cx: center,
...
var _drawTextCandidateFunc = function() {
	function byText(content, g, x, y, width, height, textAttrs, colour) {
		_setTextAttrs(g.append("text").attr("x", x + width / 2).attr("y", y + height / 2 + 5).style("font-color", colour).style("text-anchor", "middle").text(content), textAttrs);
	}
	...
	function byFo(content, g, x, y, width, height, textAttrs, conf) {
		...
		const text = body.append("foreignObject").attr("x", x).attr("y", y).attr("width", width).attr("height", height).attr("position", "fixed").append("xhtml:div").style("display", "table").style("height", "100%").style("width", "100%");
...
var drawNode = function(elem, node, fullSection, conf) {
	...
	const bbox = textElem.append("text").text(node.descr).attr("dy", "1em").attr("alignment-baseline", "middle").attr("dominant-baseline", "middle").attr("text-anchor", "middle").call(wrap, node.width).node().getBBox();
	const fontSize = conf.fontSize && conf.fontSize.replace ? conf.fontSize.replace("px", "") : conf.fontSize;
	node.height = bbox.height + fontSize * 1.1 * .5 + node.padding;
...
var getVirtualNodeHeight = function(elem, node, conf) {
	...
	const fontSize = conf.fontSize && conf.fontSize.replace ? conf.fontSize.replace("px", "") : conf.fontSize;
	textElem.remove();
	return bbox.height + fontSize * 1.1 * .5 + node.padding;
};
...
var draw = function(text, id, version, diagObj) {
	...
	const svg = (securityLevel === "sandbox" ? select_default(sandboxElement.nodes()[0].contentDocument.body) : select_default("body")).select("#" + id);
	svg.append("g");
	...
	svg.append("g").attr("class", "lineWrapper").append("line").attr("x1", LEFT_MARGIN).attr("y1", depthY).attr("x2", box.width + 3 * LEFT_MARGIN).attr("y2", depthY).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", "url(#arrowhead)");
	setupGraphViewbox$1(void 0, svg, ((_a = conf.timeline) == null ? void 0 : _a.padding) ?? 50, ((_b = conf.timeline) == null ? void 0 : _b.useMaxWidth) ?? false);
};
var drawTasks = function(diagram2, tasks2, sectionColor, masterX, masterY, maxTaskHeight, conf, maxEventCount, maxEventLineLength, maxSectionHeight, isWithoutSections) {
	...
	for (const task of tasks2) {
		...
		const taskHeight = svgDraw.drawNode(taskWrapper, taskNode, sectionColor, conf).height;
...
var drawEvents = function(diagram2, events, sectionColor, masterX, masterY, conf) {
	...
	for (const event of events) {
		...
		const eventHeight = svgDraw.drawNode(eventWrapper, eventNode, sectionColor, conf).height;
...
var genSections = (options) => {
	...
	for (let i = 0; i < options.THEME_COLOR_LIMIT; i++) {
		...
		if (isDark(options["lineColor" + i])) options["lineColor" + i] = lighten(options["lineColor" + i], 20);
		else options["lineColor" + i] = darken(options["lineColor" + i], 20);
	}
	for (let i = 0; i < options.THEME_COLOR_LIMIT; i++) {
		const sw = "" + (17 - 3 * i);
		sections2 += `
...
//# sourceMappingURL=timeline-definition-10431bc9-CknedJi3.js.map