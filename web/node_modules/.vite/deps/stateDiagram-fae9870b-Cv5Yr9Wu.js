...
import { Hn as require_dist, Rn as select_default, Rt as basis_default, T as log$1, Un as require_dayjs_min, _ as getConfig, l as configureSvgSize, s as common$1, z as utils } from "./mermaid-59c9be08-B2V2B2GS.js";
...
import { a as db, o as parser$1, s as styles } from "./styles-0c0f8aee-DDBk7hDH.js";
require_dayjs_min();
require_dist();
var idCache = {};
var set = (key, val) => {
...
var drawDescrState = (g, stateDef) => {
	const addTspan = function(textEl, txt, isFirst2) {
		...
		if (!isFirst2) tSpan.attr("dy", getConfig().state.textHeight);
	};
	const titleBox = g.append("text").attr("x", 2 * getConfig().state.padding).attr("y", getConfig().state.textHeight + 1.3 * getConfig().state.padding).attr("font-size", getConfig().state.fontSize).attr("class", "state-title").text(stateDef.descriptions[0]).node().getBBox();
	const titleHeight = titleBox.height;
	const description = g.append("text").attr("x", getConfig().state.padding).attr("y", titleHeight + getConfig().state.padding * .4 + getConfig().state.dividerMargin + getConfig().state.textHeight).attr("class", "state-description");
	let isFirst = true;
	let isSecond = true;
...
var addTitleAndBox = (g, stateDef, altBkg) => {
	...
	const titleWidth = title.node().getBBox().width + dblPad;
...
var drawNote = (text, g) => {
	...
	const { textWidth, textHeight } = _drawLongText(text, 0, 0, g.append("g"));
...
var drawEdge = function(elem, path, relation) {
	const getRelationType = function(type) {
		switch (type) {
			case db.relationType.AGGREGATION: return "aggregation";
			case db.relationType.EXTENSION: return "extension";
			case db.relationType.COMPOSITION: return "composition";
			case db.relationType.DEPENDENCY: return "dependency";
		}
	};
	...
	if (relation.title !== void 0) {
		...
		if (rows.length > 1) {
			const heightAdj = (rows.length - 1) * titleHeight * .5;
...
var draw = function(text, id, _version, diagObj) {
	...
	renderDoc(diagObj.db.getRootDoc(), diagram2, void 0, false, root, doc, diagObj);
	const padding = conf.padding;
	const bounds = diagram2.node().getBBox();
	const width = bounds.width + padding * 2;
	const height = bounds.height + padding * 2;
	configureSvgSize(diagram2, height, width * 1.75, conf.useMaxWidth);
...
var renderDoc = (doc, diagram2, parentId, altBkg, root, domDocument, diagObj) => {
	...
	graph.nodes().forEach(function(v) {
		if (v !== void 0 && graph.node(v) !== void 0) {
			...
			domDocument.querySelectorAll("#" + svgElem.id + " #" + v + " .divider").forEach((divider) => {
		...
		} else log$1.debug("No Node " + v + ": " + JSON.stringify(graph.node(v)));
	});
...
//# sourceMappingURL=stateDiagram-fae9870b-Cv5Yr9Wu.js.map