import { Rn as select_default, Rt as basis_default, T as log$1, _ as getConfig, j as sanitizeText$2, p as evaluate, u as decodeEntities, z as utils } from "./mermaid-59c9be08-B2V2B2GS.js";
import { t as line_default } from "./line-C-P71smO.js";
import { n as createText } from "./createText-31840e8c-EA-kfJEy.js";
//#region node_modules/mermaid/dist/edges-c9cd0ec5.js
...
function addHtmlLabel(node, config) {
	...
	div.html(sanitizeText$2("<span class=\"" + labelClass + "\" " + (node.labelStyle ? "style=\"" + node.labelStyle + "\"" : "") + ">" + label + "</span>", config));
...
var labelHelper = async (parent, node, _classes, isNode) => {
	...
	let text;
	if (node.labelType === "markdown") text = createText(label, sanitizeText$2(decodeEntities(labelText), getConfig()), {
		useHtmlLabels,
		width: node.width || getConfig().flowchart.wrappingWidth,
		classes: "markdown-node-label"
	});
	else text = textNode.appendChild(createLabel$1(sanitizeText$2(decodeEntities(labelText), getConfig()), node.labelStyle, false, isNode));
			...
			await Promise.all([...images].map((img) => new Promise((res) => {
				function setupImage() {
					...
					if (noImgText) {
						...
						const width = parseInt(bodyFontSize, 10) * 5 + "px";
...
var note = async (parent, node) => {
	if (!(node.useHtmlLabels || getConfig().flowchart.htmlLabels)) node.centerLabel = true;
...
var question = async (parent, node) => {
	...
	const s = bbox.width + node.padding + (bbox.height + node.padding);
	const points = [
		{
			x: s / 2,
			y: 0
...
var choice = (parent, node) => {
	...
	shapeSvg.insert("polygon", ":first-child").attr("points", points.map(function(d) {
		return d.x + "," + d.y;
...
var rect_left_inv_arrow = async (parent, node) => {
	...
	insertPolygonShape(shapeSvg, w, h, points).attr("style", node.style);
	node.width = w + h;
	node.height = h;
	node.intersect = function(point2) {
		return intersect.polygon(node, points, point2);
...
var cylinder = async (parent, node) => {
	...
	updateNodeBounds(node, shapeSvg.attr("label-offset-y", ry).insert("path", ":first-child").attr("style", node.style).attr("d", shape).attr("transform", "translate(" + -w / 2 + "," + -(h / 2 + ry) + ")"));
	node.intersect = function(point2) {
...
var stadium = async (parent, node) => {
	...
	updateNodeBounds(node, shapeSvg.insert("rect", ":first-child").attr("style", node.style).attr("rx", h / 2).attr("ry", h / 2).attr("x", -w / 2).attr("y", -h / 2).attr("width", w).attr("height", h));
	node.intersect = function(point2) {
		return intersect.rect(node, point2);
	};
	return shapeSvg;
...
var forkJoin = (parent, node, dir) => {
	...
	updateNodeBounds(node, shapeSvg.append("rect").attr("x", -1 * width / 2).attr("y", -1 * height / 2).attr("width", width).attr("height", height).attr("class", "fork-join"));
...
var class_box = (parent, node) => {
	...
	if (node.classData.type !== void 0 && node.classData.type !== "") if (getConfig().flowchart.htmlLabels) classTitleString += "&lt;" + node.classData.type + "&gt;";
	...
	node.classData.members.forEach((member) => {
		...
		if (getConfig().flowchart.htmlLabels) parsedText = parsedText.replace(/</g, "&lt;").replace(/>/g, "&gt;");
	...
	node.classData.methods.forEach((member) => {
		...
		if (getConfig().flowchart.htmlLabels) displayText = displayText.replace(/</g, "&lt;").replace(/>/g, "&gt;");
...
var insertNode = async (elem, node, dir) => {
	...
	if (node.link) {
		let target;
		if (getConfig().securityLevel === "sandbox") target = "_top";
...
var positionNode = (node) => {
	...
	log$1.trace("Transforming node", node.diff, node, "translate(" + (node.x - node.width / 2 - 5) + ", " + node.width / 2 + ")");
	const padding = 8;
...
var getSubGraphTitleMargins = ({ flowchart }) => {
	var _a, _b;
...
var positionEdgeLabel = (edge, paths) => {
	...
	if (edge.label) {
		...
		if (path) {
			...
			log$1.debug("Moving label " + edge.label + " from (", x, ",", y, ") to (", pos.x, ",", pos.y, ") abc88");
...
var intersection = (node, outsidePoint, insidePoint) => {
  ...
  node        : x:${node.x} y:${node.y} w:${node.width} h:${node.height}`);
	...
	if (Math.abs(y - outsidePoint.y) * w > Math.abs(x - outsidePoint.x) * h) {
		...
		log$1.debug(`sides calc abc89, Q ${Q}, q ${q}, R ${R}, r ${r}`, {
			_x,
			_y
		});
		if (r === 0) {
...
//#endregion
export { getLineFunctionsWithOffset as a, insertEdgeLabel as c, intersectRect$1 as d, labelHelper as f, updateNodeBounds as g, setNodeElem as h, createLabel$1 as i, insertMarkers$1 as l, positionNode as m, clear as n, getSubGraphTitleMargins as o, positionEdgeLabel as p, clear$1 as r, insertEdge as s, addEdgeMarkers as t, insertNode as u };

//# sourceMappingURL=edges-c9cd0ec5-BEHi3uVA.js.map